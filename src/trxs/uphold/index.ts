  ...
  try {    
    ...
    for (let [address, amount] of Object.entries(payments)) {        
      ...
      try {            
        ...
        monitoring.debug(`uphold: Transfering ${asset_code} equivalent to ${amount} KAS in current cycle to ${address}.`);
        await upholdTransaction(treasuryUpholdAddr, asset_code, network, upholdAddr, address, amount, transactionManager!);
        eligibleUpholdTransfer++;
      } catch(error) {
        monitoring.error(`uphold: upholdPayout - Transfering ${amount.toString()} ${asset_code} to ${address} : ${error}`);
      }
      // ⏳ Add a small delay before sending the next transaction
      await new Promise(resolve => setTimeout(resolve, 3000)); // 3s delay
    }
    monitoring.debug(`uphold: upholdPayout - Total eligible NACHO transfers: ${eligibleUpholdTransfer}`);       
  } catch (error) {
...
async function upholdTransaction(treasuryUpholdAddr: string, asset_code: string, network: string, upholdAddr:string, addressIdentifier: string, kasAmount: bigint, transactionManager: trxManager) {
  try {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        monitoring.error(`uphold: upholdTransaction - Quote API failed - ${response.status}: ${err}`);
        return;
      }
      ...
      try {
        ...
        if (!response.ok) {
          const err = await response.text();
          monitoring.error(`uphold: upholdTransaction - Transaction API failed - ${response.status}: ${err}`);
          return;
        }
        const txnData = await response.json();
        await resetBalancesByWallet(transactionManager.db, upholdAddr, kasAmount, balanceColumnMinersBalance, false);
        
        await db.recordUpholdPayment(upholdAddr, kasAmount, upholdAmount, txnData.id, asset_code, network);
        monitoring.log(`uphold: ✅ Uphold transaction completed successfully for ${upholdAddr} on ${asset_code} address - ${addressIdentifier} for network: ${network}`);
      } catch (error) {
...
export async function fetchDataForAsset(asset_code: string, accessToken: string): Promise<{ balance: string; accountId: string }> {
  try{
...
async function depositKASToUphold(address: string, amount: bigint, transactionManager: trxManager) {
  ...
  monitoring.log(`uphold: Created ${transactions.length} transactions for ${outputs.length} outputs.`);
  ...
  for (const transaction of transactions) {
    ...
    if (DEBUG) monitoring.debug(`uphold: Transaction ID ${hash} has matured. Proceeding with next transaction.`);
