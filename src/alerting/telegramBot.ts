...
export class TelegramBotAlert {
    ...
    private async checkAllBalForAlert(transactionManager: trxManager, url: string) {
        ...
        try {
          ...
          monitoring.debug(`Main: KAS balance at alert schedule: ${sompiToKAS(Number(treasuryKASBalance))} KAS`);
        } catch (error) {
        ...
        try {
          ...
          monitoring.debug(`Main: ${config.defaultTicker} balance at alert schedule: ${sompiToKAS(Number(treasuryNACHOBalance))} ${config.defaultTicker}`);
        } catch (error) {
        ...
        try {
            ...
            monitoring.debug(`Main: Amount of ${config.defaultTicker} tokens to be used for NACHO rebate: ${sompiToKAS(Number(totalKASWorthNACHO))} ${config.defaultTicker}`); 
        } catch (error) {
