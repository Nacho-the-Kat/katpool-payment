...
export default class Database {
  ...
  async getAllBalancesExcludingPool() {
    try {          
      ...
      return res.rows.map((row: { wallet: string, total_balance: string, nacho_total_balance: string, uphold_balance: string }): MinerBalanceRow => ({
  ...
  async getPayoutPreferenceFromUpholdId(uphold_id: string) {
    try {   
      ...
      const idRes = await this.pool.query(`SELECT id FROM uphold_connections WHERE uphold_id = $1`, [uphold_id]);

      if (idRes.rows.length === 0) {
        monitoring.error(`database: getPayoutPreferenceFromUpholdId - no user found for uphold_id: ${uphold_id}`);
        return null;
      }
      ...
      const res = await this.pool.query(`SELECT asset_code, network FROM uphold_payout_preferences WHERE user_id = $1`, [user_id]);

      if (res.rows.length === 0) {
        monitoring.error(`database: getPayoutPreferenceFromUpholdId - no payout preference found for user_id: ${user_id}`);
        return null;
      }
      ...
      monitoring.error(`database: getPayoutPreferenceFromUpholdId - database query error: ${error instanceof Error ? error.stack : error}`);
      return null;
    }
  }

  async recordUpholdPayment(address: string, amount: bigint, kas_amount: bigint, upholdTransactionId: string, asset_code: string, network: string) {
    try {          
      // Fetch user id from Uphold id       
      const idRes = await this.pool.query(`SELECT id FROM uphold_connections WHERE uphold_id = $1`, [address]);

      if (idRes.rows.length === 0) {
        monitoring.error(`database: getPayoutPreferenceFromUpholdId - no user found for uphold_id: ${address}`);
        return null;
      }
